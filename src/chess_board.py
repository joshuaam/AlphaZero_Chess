...
class board():
        ...
        for a, b in [(i + 1, j), (i - 1, j), (i, j + 1), (i, j - 1), (i + 1, j + 1), (i - 1, j - 1), (i + 1, j - 1),
            ...
            if 0 <= a <= 7 and 0 <= b <= 7:
                if self.current_board[a, b] in [" ", "Q", "B", "N", "P", "R"] and (a, b) not in c_list:
                    near_K = False
                    for a1, b1 in [(a + 1, b), (a - 1, b), (a, b + 1), (a, b - 1), (a + 1, b + 1),
                                   (a - 1, b - 1), (a + 1, b - 1), (a - 1, b + 1)]:
                        if 0 <= a1 <= 7 and 0 <= b1 <= 7 and self.current_board[a1, b1] == 'K':
                            near_K = True
                    if near_K is False:
                        next_positions.append((a, b))
    ...
    def in_check_possible_moves(self):
        self.copy_board = copy.deepcopy(self.current_board);
        self.move_count_copy = self.move_count  # backup board state
        self.en_passant_copy = copy.deepcopy(self.en_passant);
        self.r1_move_count_copy = copy.deepcopy(self.r1_move_count);
        self.r2_move_count_copy = copy.deepcopy(self.r2_move_count);
        self.en_passant_move_copy = copy.deepcopy(self.en_passant_move)
            ...
            for key in c_dict.keys():
                for initial_pos in c_dict[key].keys():
                    for final_pos in c_dict[key][initial_pos]:
                        ...
                        self.en_passant = copy.deepcopy(self.en_passant_copy);
                        self.en_passant_move = copy.deepcopy(self.en_passant_move_copy)
                        self.R1_move_count = copy.deepcopy(self.R1_move_count_copy);
                        self.R2_move_count = copy.deepcopy(self.R2_move_count_copy)
                        self.K_move_count = copy.deepcopy(self.K_move_count_copy);
